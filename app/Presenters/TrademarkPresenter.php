...
final class TrademarkPresenter extends Nette\Application\UI\Presenter
{
    ...
    protected function createComponentDeleteForm(): Form
    {
        ...
        {
            $this->trademarkService->delete($this->trademarkId);
    ...
    {
        $trademark = $this->trademarkService->findOneById($this->trademarkId);
        $control = $this->editTrademarkFormFactory->create($trademark);
        $control->onSave[] = function (EditTrademarkFormData $data): void
        {
            try {
                $this->trademarkService->updateIfNotExist($data, $this->trademarkId);
    ...
    {
        $this->template->trademark = $this->trademarkService->findOneById($this->trademarkId);
    }
